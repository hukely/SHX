...
<div class="content clearfix">
    <div class="item_head mt15">
		...
		<!-- Baidu Button BEGIN -->
<div id="bdshare" class="bdshare_t bds_tools get-codes-bdshare mt15" style="float:right;"> 
	<a class="bds_qzone"></a> 
...
<script type="text/javascript">
	 ...
	 */
	var bds_config = {
		'bdPic':def.tmpl+'public/images/logo.gif'//'请参考自定义分享出去的图片'
	}		
    ...
    <div class="item_list infinite_scroll" style="min-height:600px;">
        ...
        <div class="box_shadow masonry_brick mt15" style="position: absolute; top: 0px; left: 0px;">
			...
			<?php if(is_array($cate_list)): $i = 0; $__LIST__ = $cate_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><div class="catalog_item">
                    <h3><a href="<?php echo u('cate/index',array('cid'=>$val['id']));?>" <?php if($request["cid"] == $val['id']): ?>class="on"<?php endif; ?>><?php echo ($val["name"]); ?></a></h3>
                    <div class="clearfix">
                        <?php if(is_array($val['items'])): $i = 0; $__LIST__ = $val['items'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sval): $mod = ($i % 2 );++$i;?><a href="<?php echo u('cate/index',array('cid'=>$sval['id']));?>" style="color:<?php echo ($sval["color"]); ?>;" <?php if($request["cid"] == $sval['id']): ?>class="on"<?php endif; ?>><?php echo ($sval['name']); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
                    </div>

...
<div class="uc clearfix">
    ...
    <div class="nav clear-fix">
   	...
   	<?php if($u['id'] == $user['id']): ?><div class="clearfix album mt20" style="padding:0px;width:100%;">
    	<a class="upload left" href="<?php echo U('uc/share_item');?>">分享我的宝贝</a>
        <ul class="item_order left">
        	<li><i>排序：</i></li>
            <li><a <?php if($order == 'id'): ?>class="current"<?php endif; ?> href="<?php echo U('uc/share',array('order'=>'id'));?>">最新</a></li>
            <li><a <?php if($order == 'likes'): ?>class="current"<?php endif; ?> href="<?php echo U('uc/share',array('order'=>'likes'));?>">最热</a></li>
            <li><a <?php if($order == 'price'): ?>class="current"<?php endif; ?> href="<?php echo U('uc/share',array('order'=>'price'));?>">价格</a></li>
        </ul>
    </div><?php endif; ?>
        	...
        	<?php if(!empty($items_list)): if(is_array($items_list)): $i = 0; $__LIST__ = $items_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><div class="item mt15 masonry_brick jq_corner" data-corner="7px" iid="<?php echo ($val['id']); ?>">
				        <div class="item_t">
				            <div class="img tc"> 
				                ...
				                <div class="btns">
				                	<a href="javascript:;" class="img_album_btn" iid="<?php echo ($val["id"]); ?>">加入专辑</a>
				                	<a href="<?php echo u('uc/items_del',array('id'=>$val['id']));?>" class="del_btn" onclick="{if(confirm('确定要删除分享吗?')){return true;}return false;}">删除</a>              
				                </div>
				         	</div>
				            <div class="title">
						...
						<?php if(isset($val['user'])): ?><div class="user clearfix">
									<div class="clearfix">
										<div class="avatar">
											<img src="<?php echo uimg($val['user']['img']);?>" width="20px" height="20px"/> 
										</div>                
										<div class="user_info">
											<a href="<?php echo u('uc/index',array('uid'=>$val['user']['id']));?>"><em>@<?php echo ($val["user"]["name"]); ?></em></a>
											<span>
        ...
        <?php if($show_sp == 1): ?><div id="more" class="center">
                <a href="<?php echo u('uc/share',array('sp'=>2,'cid'=>$cid,'p'=>$p));?>" hidefocus="true"></a>
            </div><?php endif; ?> 

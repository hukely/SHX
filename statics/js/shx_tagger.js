...
function shx_tagger_bookmarklet(){
   ...
   this.iframe_url = "http://www.huhongfei.com/SHX/item/tagger";
   this.css_url = 'http://www.huhongfei.com/SHX/statics/css/bookmarklet.css'
   ...
   this.runthis = function(){
     ...
     this.theindex_user_key = theindex_username;
     ...
     imgDivNode.style.background = "url( 'http://s3.amazonaws.com/thefancy/_ui/images/f-plus.png')   no-repeat scroll 5px 5px transparent";
	  ...
	  try {
         ...
         var msg = {
		  client_url: document.URL,
                  user_key: this.theindex_user_key,
	   ...
	   if(window.location.hash != this.tf_current_hash) {
		   ...
		   fttaggerbookmarklet.setScroll(scroll)
			if (json_over_url.close_iframe != undefined && json_over_url.close_iframe==1){
			   var script_id="theindex_tagger_bookmarklet_helper_js";
...
if (document.URL.indexOf(".shx.com") == -1){
   fttaggerbookmarklet = new shx_tagger_bookmarklet();
...
if (document.URL.indexOf(".shx.com") == -1){
   window.theindex_bookmarklet = {};
   window.theindex_bookmarklet.tagger = fttaggerbookmarklet;
}
